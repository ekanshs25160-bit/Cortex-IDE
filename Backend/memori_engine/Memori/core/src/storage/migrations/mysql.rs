...
pub static MIGRATIONS: &[(u32, &[Migration])] = &[
    (
        ...
        &[
            ...
            Migration {
                ...
                statements: &["create table if not exists memori_entity_fact(\
                        id bigint not null auto_increment,\
                        uuid varchar(36) not null,\
                        ...
                        constraint fk_memori_ent_summ_entity foreign key (entity_id) references memori_entity (id) on delete cascade\
                    )"],
            },
            Migration {
            ...
            Migration {
                ...
                statements: &["create table if not exists memori_knowledge_graph(\
                        ...
                        unique key (uuid),\
                        constraint fk_memori_know_graph_entity foreign key (entity_id) references memori_entity (id) on delete cascade,\
                        constraint fk_memori_know_graph_object foreign key (object_id) references memori_object (id) on delete cascade,\
                        constraint fk_memori_know_graph_predicate foreign key (predicate_id) references memori_predicate (id) on delete cascade,\
                        constraint fk_memori_know_graph_subject foreign key (subject_id) references memori_subject (id) on delete cascade\
                    )"],
            },
        ],
    ),
    (
        ...
        &[Migration {
            ...
            statements: &["create table if not exists memori_entity_fact_mention(\
                    id bigint not null auto_increment,\
                    uuid varchar(36) not null,\
                    entity_id bigint not null,\
                    ...
                    constraint fk_memori_ent_fact_mention_entity_fact foreign key (entity_id, fact_id) references memori_entity_fact (entity_id, id) on delete cascade,\
                    constraint fk_memori_ent_fact_mention_conversation foreign key (conversation_id) references memori_conversation (id) on delete cascade\
                )"],
        }],
    ),
];
