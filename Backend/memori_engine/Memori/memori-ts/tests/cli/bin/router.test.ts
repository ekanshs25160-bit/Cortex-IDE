...
describe('CLI Router', () => {
  ...
  it('should catch unexpected errors, log them, and exit gracefully', async () => {
    vi.mocked(signupCommand).mockRejectedValueOnce(new Error('Explosion!'));

    process.argv = ['node', 'cli.js', 'sign-up'];
    await main();

    expect(consoleErrorSpy).toHaveBeenCalledWith(
      expect.stringContaining("Unexpected error executing command 'sign-up'")
    );
