...
Each fact, preference, or relationship extracted by Advanced Augmentation counts as one memory. A single conversation or agent trace session can generate multiple memories.
...
It runs in the background after each conversation or agent trace and extracts facts, preferences, and relationships while minimizing impact on your LLM response path. See [Advanced Augmentation](/docs/memori-cloud/concepts/advanced-augmentation).
